    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        setStats([
          ...
          { name: 'Revenue', value: 'Error', icon: CurrencyDollarIcon, color: 'yellow' }
        ]);
      } finally {
        setLoading(false);
      }
            ...
            stats.map((stat) => {
                ...
                return (
                  <Link to={linkTo} key={stat.name} className="block">
                    <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200 flex items-center h-28 hover:shadow-md hover:scale-[1.02] transition-all duration-200 cursor-pointer">
